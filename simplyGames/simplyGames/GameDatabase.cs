...
namespace SimplyGames
{
    public class GameDatabase
    {
        ...
        public List<Order> getOrderData()
        {
            ...
            try
            {
                command.CommandText = "Select * from GameOrder";
                command.CommandType = CommandType.Text;
                connection.Open();
                OleDbDataReader reader = command.ExecuteReader();
                while (reader.Read())
                {
                    Order o = new Order();
                    o.OrderNumber = Convert.ToInt32(reader["OrderNumber"].ToString());
                    o.Customer = reader["CustomerName"].ToString();
                    o.Game = reader["GameName"].ToString();
                    o.Platform = reader["GamePlatform"].ToString();
                    o.Price = Convert.ToDecimal(reader["GamePrice"].ToString());
                    o.StaffMember = Convert.ToInt32(reader["StaffMember"].ToString());              
                    Orders.Add(o);
                }
                return Orders;
            }
        ...
        internal void insertOrder(string strOrderNo, string strCust, string strGame, string strPlatform, string strPrice, string strStaff)
        {
            try
            {
                command.CommandText = "INSERT INTO GameOrder (OrderNumber, CustomerName, GameName, GamePlatform, GamePrice, StaffMember) VALUES ('" + strOrderNo + "', '" + strCust + "', '" + strGame + "', '" + strPlatform + "', '" + strPrice + "', '" + strStaff + "')";
